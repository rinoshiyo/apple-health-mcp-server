...
- **MCP ツール名とパラメータシグネチャ** — 新ツール追加はマイナー
  バンプ、 既存ツールのリネーム / 削除 / パラメータリスト変更は
  メジャーバンプ
- **DuckDB スキーマのカラム名・型** — カラム追加はマイナー、
  リネーム / 削除 / 型変更はメジャー（`run_custom_query` でテーブルへ
  SQL を直接書く利用者向け）
- **CLI サブコマンド名と必須フラグ** — 同様

`src/apple_health_mcp/` 配下の internal モジュール、 および `_` プレ
フィックス付き識別子（ヘルパー、 private 定数、 internal 例外）は
