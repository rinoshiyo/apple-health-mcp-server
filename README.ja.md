...
（Claude Desktop を含む）へ公開します。読み取り中心の 18 ツールで構成されます。
...
4. **Windows (issue #128):** install ダイアログに **Database file**
   フィールドが現れます。 `%LOCALAPPDATA%` の **外側** に安定した
   パスを指定してください (例: `C:\Users\<you>\apple-health\health.duckdb`)。
   ターミナルから `import` を走らせる時も `--db` で **同じ** パスを
   渡します。 理由: Windows 版 Claude Desktop は MSIX パッケージで、
   子プロセスは AppContainer サンドボックス内で動き `%LOCALAPPDATA%`
   が per-package 専用パスに virtualise されます。 ターミナル側の
   importer はサンドボックス外、 サンドボックス内のサーバはその内側
   を見るため、 共通の安定パスが無いと「import 成功したのに全 tool が
   no data を返す」状態になります。 macOS / Linux ではこのリダイレクトが
   無いのでフィールドは空のままで OK です。
...
2. `APPLE_HEALTH_DB` 環境変数 (ファイルパス) — MCPB バンドルの
   **Database file** GUI フィールドはこの env に注入されます。 `--db`
   と同じ優先度: CLI 側で `--db` を APPLE_HEALTH_DB に promote
...
FastMCP に登録される 18 ツールを系統別にまとめます。
...
復旧は一度きりの再インポートで完了します。

```bash
# v0.3.0 未満の DB を削除（既定パス。 --db で上書きしている場合はそのパス）。
rm ~/.local/share/apple-health-mcp/health.duckdb

# 直近の Apple ヘルスケア export.zip を展開したディレクトリから再インポート。
