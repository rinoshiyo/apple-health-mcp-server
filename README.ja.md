...
（Claude Desktop を含む）へ公開します。MCP ツールは合計 20 個（読み取り 18 + ZIP フロー 2）で構成されます。
...
ているため "MiB" 表記）。ストリーミングで監視している AI エージェントや
...
FastMCP に登録される 20 ツールを系統別にまとめます。
...
```bash
# v0.3.0 未満の DB を削除（既定パスは [データベースの場所](#database-location-ja)
# を参照、 --db で上書きしている場合はそのパス）。
