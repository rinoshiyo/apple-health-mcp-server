...
DESCRIPTION = (
    "Query individual health records. Returns `{items, total, next_offset}`; "
    "`next_offset` is `null` on the last page. Each item carries: "
    "record_hash, record_type, value (numeric measurement), text_value "
    "(categorical value, e.g. sleep stages like "
    "HKCategoryValueSleepAnalysisAsleepDeep), unit, source_name, "
    "start_date, end_date. Record types use Apple's HK identifiers "
    "(e.g. HKQuantityTypeIdentifierHeartRate, "
    "HKCategoryTypeIdentifierSleepAnalysis). Use list_record_types first "
...
def register(mcp: FastMCP, conn: duckdb.DuckDBPyConnection, lock: Lock) -> None:
    ...
    async def query_records(
        ...
        source_name: Annotated[
            str | None,
            Field(description="Filter by source name", max_length=256),
        ] = None,
        limit: Annotated[
            int | None,
