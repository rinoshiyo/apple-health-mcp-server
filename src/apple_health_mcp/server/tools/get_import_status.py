...
DESCRIPTION = (
    ...
    "is the worker-thread wall-clock from queued → done, INCLUDING ZIP "
    "extraction; for the run_import-body-only figure (XML + ECG + GPX + "
    "finalize, excluding extraction) use "
    "``get_import_history.processing_secs``. The two values can differ "
    "by several seconds on large ZIPs. Or {status: 'error', "
